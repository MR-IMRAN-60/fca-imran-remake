...
try {
    if (!global.Fca.Require.fs.existsSync(process.cwd() + '/ImranFca.json')) {
        global.Fca.Require.fs.writeFileSync(process.cwd() + "/ImranFca.json", JSON.stringify(global.Fca.Data.ObjPriyansh, null, "\t"));
        process.exit(1);
    }

try {
    var DataLanguageSetting = require(process.cwd() + "/ImranFca.json");
}
catch (e) {
    ...
    if (global.Fca.Require.fs.existsSync(process.cwd() + '/PriyanshFca.json')) {
        try { 
            if (DataLanguageSetting.Logo != undefined) {
                ...
                global.Fca.Require.fs.writeFileSync(process.cwd() + "/ImranFca.json", JSON.stringify(DataLanguageSetting, null, "\t"));        
            }
        }
        catch (e) {
            console.log(e);
                    ...
                    Fetch('https://raw.githubusercontent.com/corazoncary/fca/main/package.json').then(async (/** @type {{ body: { toString: () => string; }; }} */res) => {
                        const localVersion = global.Fca.Version;
                            if (Number(localVersion.replace(/\./g,"")) < Number(JSON.parse(res.body.toString()).version.replace(/\./g,"")) ) {
                                log.warn("[ FCA-IMRAN ] •",getText(Language.NewVersionFound,global.Fca.Version,JSON.parse(res.body.toString()).version));
                                if (global.Fca.Require.Priyansh.AutoUpdate == true) { 
                                    log.warn("[ FCA-IMRAN ] •",Language.AutoUpdate);
                                        try {
                                                ...
                                                logger.Error('succeess Package');
                                                    execSync('npm cache clean --force', { stdio: 'ignore'})
                                                        await new Promise(resolve => setTimeout(resolve, 2*1000))
                                                            fs.removeSync('../fca-imran');
                                                                // why stdio is not studio :v 
                                                            await new Promise(resolve => setTimeout(resolve, 2*1000))
                                                        execSync('npm i fca-imran-ramke@latest', { stdio: 'inherit'})
                                                    logger.Success("success Restart");
                                                process.exit(1);
                                            }
                                            catch (e) {
                                                logger.Warning("Error Please Enter The Following Code In Console To Fix !");
                                                    logger.Warning("rmdir -rf ./node_modules/fca-imran-remake && npm i fca-imran-remake@latest && npm start");
        ...
        rl.question(Language.TypeAccount, (Account) => {
            if (!Account.includes("@") && global.Fca.Require.utils.getType(parseInt(Account)) != "Number") return logger.Normal(Language.TypeAccountError, function () { process.exit(1) }); //Very Human
                    ...
                    if (global.Fca.Require.Priyansh.ResetDataLogin) {
                        ...
                        global.Fca.Require.fs.writeFileSync(process.cwd() + '/ImranFca.json', JSON.stringify(global.Fca.Require.Priyansh, null, 4));
                    }
